// mLink_Button_Pad_Read_LCD_Task_PJON_Data.ino 
...
// Add sending Keypad and Button data.
// The display data is now sent to the display on the receiver.
...
{  
  ...
  if(!empty)                                          
  { 
    byte button = mLink.read(I2C_ADD_BPAD, BPAD_BUFFER);      // If so then read a key from the buffer and print it
    ...
    mLink.cLCD_print(I2C_ADD_LCD, button);       // Print key value
    payload[0] = 'B';
    payload[1] = button;
    bus.send(44, payload, 2); // Send button to device 44
  }
